...
export default function PrivateRoute({ children }) {
  ...
  // 🔥 Авто-проверка верификации каждые 3 сек
  useEffect(() => {
    ...
    const interval = setInterval(async () => {
      ...
      if (user.emailVerified) {
        ...
        window.location.reload() // 🔥 Перезагружаем чтобы пустило
      }
    }, 3000)
  ...
  if (user && !user.emailVerified) {
    return (
      <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', height: '100vh', gap: 16, fontFamily: 'var(--mono)', color: '#888', textAlign: 'center', padding: 20 }}>
