                ...
                {planData?.form && (
                  <div className={styles.dataCard}>
                    <div className={styles.dataHeader}>
                      ...
                      <button className={styles.editBtn} onClick={() => { setEditing(e => !e); setEditErrors({}) }}>
                    ...
                    {!editing ? (
                      <div className={styles.dataGrid}>
                        ...
                        <div className={styles.dataItem}>
                          ...
                          <div className={styles.dataVal} style={{fontSize:'14px'}}>{planData.form.equipment || '—'}</div>
                        </div>
                        {planData.form.freeText && (
                          <div className={styles.dataItem} style={{ gridColumn: '1/-1' }}>
                            <div className={styles.dataLabel}>Дополнительно</div>
                            <div className={styles.dataVal} style={{fontSize:'14px'}}>{planData.form.freeText}</div>
                          </div>
                        )}
                      </div>
                    ) : (
                      <div className={styles.editGrid}>
                        ...
                        <div className={styles.editField} style={{ gridColumn: '1/-1' }}>
                          ...
                          <div className={styles.btnRow}>
                            {['Набор массы','Сила','Рельеф','Выносливость','Похудение'].map(g => (
                              <button key={g}
                        ...
                        <div className={styles.editField} style={{ gridColumn: '1/-1' }}>
                          <label className={styles.editLabel}>Дополнительно <span style={{color:'#555',fontSize:'10px'}}>(необязательно)</span></label>
                          <textarea
                            className={styles.editInput}
                            ...
                            rows={3}
                            style={{ resize: 'vertical', lineHeight: '1.6', padding: '10px' }}
                          />
                        </div>
                      ...
                      {content && isList && (
                        <div className={styles.tipsList}>
                          {(Array.isArray(content) ? content : content.split('\n'))
                            .filter(l => l && l.trim())
                            .map((line, i) => <TipItem key={i} text={typeof line === 'string' ? line.trim() : String(line)} />)}
                        </div>
                      )}
                    </div>
                  </div>
            ...
            <div className={styles.modalOverlay} onClick={() => setShowRePlanModal(false)}>
              <div className={styles.replanModal} onClick={e => e.stopPropagation()}>
                ...
                <div className={styles.replanBtns}>
                  <button className={styles.replanYes} onClick={() => { setShowRePlanModal(false); navigate('/generate') }}>
                    Да, пересоздать
                  </button>
